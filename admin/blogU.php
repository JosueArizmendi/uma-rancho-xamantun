...
// Obtener datos del blog a editar
...
if (!$blog) {
    header("Location: blogsR.php?error=blog_not_found");
    exit;
}
...
<body>
    ...
    <main class="fixed-top-offset">
        <div class="container">
            <h4 class="text-center py-2">Editar Blog</h4>
            
            <?php if (isset($_GET['error'])): ?>
                <div class="alert alert-danger">
                    Error al actualizar el blog. Por favor, inténtalo de nuevo.
                </div>
            <?php endif; ?>

            <div class="card card-default border:3 shadow p-3 mb-5">
                <form action="../model/modificar_blog.php?id_blog=<?= base64_encode($blog['id_blog']) ?>" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                    <div class="row row-cols-1">
                        <div class="mb-3">
                            <label class="form-label" for="titulo">Título</label>
                            <input type="text" class="form-control" id="titulo" name="titulo" value="<?= htmlspecialchars($blog['titulo']) ?>" required>
                            <div class="valid-feedback">Campo correcto!</div>
                            <div class="invalid-feedback">Por favor ingresa un título</div>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label" for="descripcion">Artículo</label>
                            <textarea class="form-control" id="descripcion" name="descripcion" rows="5" required><?= htmlspecialchars($blog['descripcion']) ?></textarea>
                            <div class="valid-feedback">Campo correcto!</div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" value="<?= htmlspecialchars($blog['nombre_autor']) ?>" readonly>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label">Fecha de publicación</label>
                            <input type="text" class="form-control" value="<?= date('d/m/Y', strtotime($blog['fecha_publicacion'])) ?>" readonly>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label">Imágenes (Máximo 3)</label>
                            <div class="row">
                                <div class="col-md-4">
                                    <label for="imagen1" class="form-label">Imagen 1</label>
                                    <?php if ($blog['imagen1']): ?>
                                        <img src="<?= $blog['imagen1'] ?>" class="preview-img">
                                        <input type="hidden" name="imagen1_actual" value="<?= $blog['imagen1'] ?>">
                                    <?php endif; ?>
                                    <input type="file" class="form-control mt-2" id="imagen1" name="imagen1" accept="image/*" onchange="previewImage(this, 'preview1')">
                                    <img id="preview1" class="preview-img" style="display: none;">
                                </div>
                                <div class="col-md-4">
                                    <label for="imagen2" class="form-label">Imagen 2</label>
                                    <?php if ($blog['imagen2']): ?>
                                        <img src="<?= $blog['imagen2'] ?>" class="preview-img">
                                        <input type="hidden" name="imagen2_actual" value="<?= $blog['imagen2'] ?>">
                                    <?php endif; ?>
                                    <input type="file" class="form-control mt-2" id="imagen2" name="imagen2" accept="image/*" onchange="previewImage(this, 'preview2')">
                                    <img id="preview2" class="preview-img" style="display: none;">
                                </div>
                                <div class="col-md-4">
                                    <label for="imagen3" class="form-label">Imagen 3</label>
                                    <?php if ($blog['imagen3']): ?>
                                        <img src="<?= $blog['imagen3'] ?>" class="preview-img">
                                        <input type="hidden" name="imagen3_actual" value="<?= $blog['imagen3'] ?>">
                                    <?php endif; ?>
                                    <input type="file" class="form-control mt-2" id="imagen3" name="imagen3" accept="image/*" onchange="previewImage(this, 'preview3')">
            ...
            <div style="padding-bottom:0.5cm" class="row w-100 align-items-center">
                <div class="col text-center">
                    <a href="blogsR.php" class="btn btn-success bi bi-arrow-return-left"></a>
                </div>
            </div>
        </div>
