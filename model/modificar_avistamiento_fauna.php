...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_FILES['input_imagen']) && $_FILES['input_imagen']['error'] === UPLOAD_ERR_OK) {
            ...
            $stmt = $conn->prepare("SELECT ruta_imagen FROM avistamientos_animales WHERE id_avistamiento = :id");
        ...
        $sql = "UPDATE avistamientos_animales SET 
        ...
        if ($stmt->execute()) {
            $_SESSION['notification'] = [
                'type' => 'success',
                'title' => '¡Éxito!',
                'message' => 'El avistamiento de fauna fue actualizado correctamente.'
            ];
        } else {
            $_SESSION['notification'] = [
                'type' => 'danger',
                'title' => 'Error',
                'message' => 'No se pudo actualizar el avistamiento de fauna.'
            ];
        }
    ...
    $sql = "SELECT * FROM avistamientos_animales WHERE id_avistamiento = :id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id', $codigo);
    $stmt->execute();
